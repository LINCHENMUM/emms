...
/*           XTree           */
/*****************************/
window.onerror=function(){	parent.ymPrompt.alert({message:error,handler:null});
	return true;	
}
var bCanSelect = 'true';
var webRoot = '../images/xloadtree/';
window.onerror=function(){
	parent.ymPrompt.alert({message:error,handler:null});
	return true;	
}
var webFXTreeConfig = {
	rootIcon        : webRoot+'foldericon.gif',
	openRootIcon    : webRoot+'ball.gif',
	folderIcon      : webRoot+'foldericon.gif',
	openFolderIcon  : webRoot+'openfoldericon.gif',
	fileIcon        : webRoot+'file.gif',
	iIcon           : webRoot+'I.gif',
	lIcon           : webRoot+'L.gif',
	lMinusIcon      : webRoot+'Lminus.gif',
	lPlusIcon       : webRoot+'Lplus.gif',
	tIcon           : webRoot+'T.gif',
	tMinusIcon      : webRoot+'Tminus.gif',
	tPlusIcon       : webRoot+'Tplus.gif',
	blankIcon       : webRoot+'blank.gif',
	defaultText     : webRoot+'Tree Item',
	defaultAction   : 'javascript:parentNode();',
...
WebFXCookie.prototype.setCookie = function (key, value) {
	//document.cookie = key + "=" + escape(value); //dl delete
	 if (this.cookies) { 
...
function WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon) {
	...
	} else { this.open  = false; }//fuwei true
	this.folder    = true;
	this.rendered  = false;
	this.onSelect  = null;
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	if(bCanSelect=='true'&&this.radio=="true"){
		if(this.checked=="true"){		
			selectedstr="checked";
		}
		str+="<input type=\"Radio\" id=\"selRadioObj\" name=\"selRadioObj\" value=\""+svalue+"\" onclick=\"selectRadio(this)\" "+selectedstr+">";
	}else if(bCanSelect=='true'&&this.checkbox=="true"){
		if(this.checked=="true"){		
			selectedstr="checked";
		}
...
XmlHttp.create = function () {
	...
	// fell through
	throw new Error("您的浏览器不支持XMLHTTP，建议使用IE6.0");
};
...
XmlDocument.create = function () {
	...
	throw new Error("您的浏览器不支持XMLDocument，建议使用IE6.0");
};
...
webFXTreeConfig.loadErrorTextTemplate = "加载\"%1%\"的子节点失败,请与管理员联系";
webFXTreeConfig.emptyErrorTextTemplate = "提示：\"%1%\" 没有子节点！";
webFXTreeConfig.loadingIcon = "/iffip/_images/xloadtree/loading.gif"; // dl add image
...
function _startLoadXmlTree(sSrc, jsNode) {
	...
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4) {
			_xmlFileLoaded(xmlHttp.responseXML, jsNode,sSrc);//fuwei update
		}
	};
...
function _xmlFileLoaded(oXmlDoc, jsParentNode,sSrc) {
	...
	if (bIndent) {
		...
		//jsParentNode.indent();//fuwei xiugai 
	}
